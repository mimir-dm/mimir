...
# Replace string-based cross-crate errors with proper error types
...
Replace string-based error variants for Print and LLM errors in MimirError with proper error types that preserve context and enable pattern matching.
...
- [ ] `MimirError::Print` variant holds `PrintError` or `Box<dyn Error>` instead of `String`
- [ ] `MimirError::Llm` variant holds `LlmError` or `Box<dyn Error>` instead of `String`
- [ ] Error chain preserved when converting from child crate errors
- [ ] All error handling code compiles and tests pass
