...
updated_at: 2026-03-10T19:18:02.269887+00:00
parent: 
blocked_by: []
archived: true
...
1. **Tauri commands** (done in session 1): `add_character_spell`, `remove_character_spell`, `toggle_spell_prepared` added to `character.rs` and registered in `main.rs`
2. **Remove spell command** (done in session 1): Supports removing by class or all instances
3. **MCP tools**: Added `add_character_spell_tool`, `remove_character_spell_tool`, `list_character_spells_tool` to `crates/mimir-mcp/src/tools/character.rs` with implementations. Registered in handler. Updated expected tool count test (64→67).
4. **Level-up spell selection**: Modified `useSpellManagement.ts` `loadClassSpells()` to also fetch homebrew spells via `list_homebrew_spells` and merge them into the spell list with `source: 'HB'`
5. **Character sheet display**: Added "HB" badge tag to `SpellsSection.vue` for spells with `source === 'HB'`, styled with primary color
6. **Spell card PDF export**: Modified `crates/mimir/src/commands/print/character.rs` to fetch homebrew spells from campaign via `dal::list_campaign_homebrew_spells` and include them in `spell_data` for PDF generation
7. **Toggle prepared** (done in session 1): `toggle_spell_prepared` command works for all spells regardless of source

All Rust code compiles cleanly. All tests pass.