...
updated_at: 2026-01-20T20:58:51.804739+00:00
parent: MIMIR-I-0042
blocked_by: []
archived: true
...
Implemented `CatalogImportService` in `mimir-core/src/import/service.rs` with full functionality:

**Core Features:**
- `CatalogImportService` struct with database connection
- `import_from_directory()` discovers and imports all available source books
- Per-source transaction handling using SAVEPOINT/RELEASE/ROLLBACK
- Fail-and-continue error handling per source book
- `ImportResult` struct with summary of imported/failed sources and entity counts

**Entity Import:**
- All 20 entity types supported (monster, spell, item, class, subclass, race, background, feat, condition, disease, action, language, vehicle, object, trap, hazard, cult, deity, sense, skill)
- Field extraction for indexed columns:
  - Monster: CR, creature_type, size
  - Spell: level, school, ritual, concentration
  - Item: type, rarity
  - Language/Vehicle/Object: type fields
  - Trap: tier
  - Deity: pantheon
  - Skill: ability
...
**Join Tables:**
- `spell_classes` - populated from `classes.fromClassList`
- `spell_subclasses` - populated from `classes.fromSubclass`
- `item_attunement_classes` - extracted from `reqAttune` string

**FTS Indexing:**
- Indexes entities during import using `index_entity_fts()`
- Flattens `entries` array for rules content
- Flattens `fluff.entries` for fluff content
- Uses ContentType::Rules and ContentType::Fluff

**Testing:**
- 302 tests passing
- Unit tests for field extraction helpers
- Unit tests for ImportResult summary

**Note:** Implemented in `mimir-core` crate since `mimir-services` doesn't exist in this project structure.