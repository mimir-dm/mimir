...
mod tests {
    ...
    fn test_tiled_map_one_page_per_tile() {
        ...
        // Verify block(breakable: false) is used to keep label and image together
        assert!(
            typst.contains("#block(breakable: false)") || typst.contains("block(breakable: false)"),
            "Tiles should use non-breakable blocks to keep label and image together"
        );
    }
}
