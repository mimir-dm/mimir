//! Shared Tauri invoke handler
//!
//! Single source of truth for the command registration list, generic over the
//! Tauri runtime so both the production app (Wry) and the dev-only UI harness
//! bridge (MockRuntime) dispatch through the exact same `generate_handler!`
//! pipeline. Adding a command means adding one line here — every consumer
//! picks it up.
...
/// Build the invoke handler covering every Mimir command.
