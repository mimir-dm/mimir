<template>
  ...
  >
    <div class="sources-modal">
      <p class="description">
        Select which source books are available for this campaign. This controls which
        monsters, items, and other content the DM can use.
      </p>
...
// Core D&D 5e books
const CORE_SOURCES = ['PHB', 'XPHB', 'DMG', 'XDMG', 'MM', 'XMM', 'PHB2024', 'DMG2024', 'MM2024']
...
async function loadData() {
  ...
  try {
    ...
    // Load campaign's selected sources
    const campaignResult = await invoke<{ success: boolean; data?: string[] }>('list_campaign_sources', {
      campaignId: props.campaignId
    })
    if (campaignResult.success && campaignResult.data) {
      selectedSources.value = new Set(campaignResult.data)
      originalSources.value = new Set(campaignResult.data)
    } else {
...
async function save() {
  saving.value = true
  try {
    const result = await invoke<{ success: boolean; error?: string }>('set_campaign_sources', {
      campaignId: props.campaignId,
